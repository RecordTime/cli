...
class Core extends Tapable {
  ...
  startRest() {
    render.renderCoffee();
    const timer = new Timer({ minutes: 25, duration: 3000 });
    timer.hooks.endTick.tap('Cli', () => {
      this.startWork();
    });
    timer.start();
  }
  ...
  async startWork() {
    const pomodoro = new Timer({ minutes: 25, duration: 5000 });
    pomodoro.hooks.startTick.tap('Cli', (beginTime) => {
      render.renderWork(beginTime.toLocaleTimeString());
    });
    // pomodoro.hooks.tick.tap('Cli', (current) => {
    //   if (current === 10000) {
    //     notifier.notify({
    //       title: 'RecordTime',
    //       message: '欢乐时光就要开始了~',
    ...
    //   }
    // });
    pomodoro.hooks.endTick.tap('Cli', async (beginTime, endTime) => {
      open();
      render.renderTomato(`${beginTime.toLocaleTimeString()} ~ ${endTime.toLocaleTimeString()}`);
      // 弹出关联任务
      this.displayByBoard();
