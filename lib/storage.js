...
class Storage {
  constructor() {
    ...
    this._logFile = path.join(this._storageDir, LOG_FILE_NAME);
   ...
   */
  getLogs() {
    return this.readFileContent(this._logFile);
  }
  ...
  setLogs(logs) {
    ...
    const tempStorageFile = this._getTempFilePath(this._logFile);
    fs.writeFileSync(tempStorageFile, content, 'utf8');

    fs.renameSync(tempStorageFile, this._logFile);
    return true;
  }
